...
async function test() {
  ...
  // 创建表并插入数据
  result = await sqlite3.exec('CREATE TABLE test (id INTEGER, name TEXT); INSERT INTO test VALUES (1, "Alice"), (2, "Bob"); SELECT count(*) count FROM test;')
  asserts(result[0].count === 2, '查询结果数量不正确');
